...
mysql_select_db($database_ConexaoIdec, $ConexaoIdec);
$query_Recordset1 = sprintf("SELECT * FROM idec_matriculas WHERE id = %s", GetSQLValueString($colname_Recordset1, "int"));
...
<table width="1016" height="246" border="0" align="center" style="background:url(../img/imgFundoExcluirMatriculas.png) no-repeat">
  <tr>
    <td width="978" height="242"><form id="form1" name="form1" method="post" action="">
      <table width="712" height="213" align="center">
        <tr>
          <td height="181" colspan="3"><p>Tem certeza que deseja excluir essa MATRÍCULA:
              <?php echo $row_Recordset1['cod']; ?></p>
            <p><?php echo $row_Recordset1['nomeEstudante']; ?></p>
            <p><?php echo date('d/m/Y', strtotime($row_Recordset1['inicioCurso']));?> - <?php echo date('d/m/Y', strtotime($row_Recordset1['terminoCurso']));?></p></td>
        </tr>
        <tr>
          ...
          <td width="98"><input type="submit" name="button" id="button" value="SIM" />
            <input name="button2" type="submit" id="button2" onclick="MM_goToURL('parent','../admin/matriculas.php');return document.MM_returnValue" value="NÃO" /></td>
          <td width="216">&nbsp;</td>
        </tr>
      </table>
    </form></td>
